...
$mail = 'guanfangyouxiang@gmail.com';//若有恶意登录，会发邮件到这个邮箱，前提是mail()函数可用！
...
function login($er=false) {
    ...
    if ($er) { 
        if (isset($_SESSION['error'])){
            $_SESSION['error']++;
            if($_SESSION['error'] > $safe_num && $safe_num !== 0){
                @mail($mail,'Files文件管理器提醒：文件被恶意登录！','该提醒来自Files：<br>登录者IP为：'.$_SERVER['REMOTE_ADDR'],'From: <i@hezi.be>');
    ...
    echo "<form action=\"".$meurl."?op=".$op."\" method=\"post\">\n"
        ."<input type=\"text\" name=\"user\" border=\"0\" class=\"login_text\" value=\"".@$user."\"  placeholder=\"Username\">\n"
        ."<input type=\"password\" name=\"pass\" border=\"0\" class=\"login_text\" value=\"".@$pass."\" placeholder=\"Password\">\n"
...
function mainbottom() {
    echo "</div><div style='text-align:center;font-size:13px;color:#999 !important;margin-top:5px;margin-bottom:45px'>"
        ."Files Version 1.0 "."(From ip:".getIP().")</div></body>\n"
        ."</html>\n";
    exit;
}
